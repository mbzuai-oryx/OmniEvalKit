...
ANNOTATION_FILES = ("test_Video-Holmes.json", "video_holmes.jsonl")
eval_type = "closed"
...
def _normalize_record(record, root, base_dir, idx):
    ...
    video_id = _first(record, ("video_id", "video ID", "uid"))
    if not video_value and video_id:
        video_value = f"videos_cropped/{video_id}.mp4"
    ...
    sample_id = _first(record, ("id", "sample_id", "question_id", "Question ID", "video_id", "uid")) or f"{DATASET_NAME}_{idx}"
    ...
    return {
        ...
        "options": _normalize_options(_first(record, ("options", "choices", "Choice", "Choices", "Options"))),

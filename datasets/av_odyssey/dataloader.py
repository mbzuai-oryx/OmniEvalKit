...
ANNOTATION_FILES = ('av_odyssey.jsonl',)
eval_type = "closed"
...
def _normalize_record(record, root, base_dir, idx):
    ...
    image_values = _media_values(record, ("image_path", "image", "ImagePath", "imagePath"), ("image_paths_dict",))
    ...
    image_paths = [_resolve_path(root, base_dir, value) for value in image_values]
    ...
    return {
        ...
        "image_path": image_paths if len(image_paths) > 1 else (image_paths[0] if image_paths else None),
    }
...
def _media_value(record, keys, dict_keys):
    values = _media_values(record, keys, dict_keys)
    return values[0] if values else None


def _media_values(record, keys, dict_keys):
    ...
    if isinstance(value, dict):
        return [item for _, item in sorted(value.items()) if item not in (None, "")]

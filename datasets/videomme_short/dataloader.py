...
DATASET_NAME = "videomme_short"
ANNOTATION_FILES = ('videomme_short.jsonl',)
eval_type = "closed"
...
def _normalize_record(record, root, base_dir, idx):
    ...
    video_value = _media_value(record, ("video_path", "video", "VideoPath", "videoPath", "url"), ("video_paths_dict",))
...
def _normalize_options(options):
    ...
    if isinstance(options, dict):
        ...
        normalized_text = " ".join(options.replace("[", " ").replace("]", " ").split())
        matches = re.findall(
            r"(?:^|\s)[\"']?([A-D][\.\)]\s*.*?)(?=\s+[\"']?[A-D][\.\)]|\s*$)",
            normalized_text,
        )
        options = matches or re.split(r"\n|\|", options)
...
def _strip_option_prefix(option):
    text = "" if option is None else str(option).strip().strip("\"'")

...
class Model:
    ...
    def __init__(self, model_path=None):
        ...
        self.default_checkpoint = self.package_dir / "model_checkpoint_nvila_video"
    ...
    def _default_model_path(self):
        ...
        return "Efficient-Large-Model/NVILA-8B-Video"
    ...
    def _load(self):
        ...
        try:
            ...
            raise ImportError(
                "VILA_whisper_1 requires the bundled NVIDIA VILA source and its runtime dependencies."
            ) from exc
        ...
        if not hasattr(self.model, "generation_config"):
            self.model.generation_config = self.model.llm.generation_config
        self.model.config.num_video_frames = self.num_video_frames
        if self.max_tiles_video > 0 and hasattr(self.model.config, "video_max_tiles"):
            self.model.config.video_max_tiles = self.max_tiles_video
    ...
    def _add_local_dependency_paths(self):
        for path in (self.local_python_deps, self.local_s2_source, self.local_vila_source):
            if path.exists():
                value = str(path)
    ...
    def run_inference(self, audio_path, video_path, query, system_prompt, image_path=None, sample=None):
        ...
        prompt = [Image(path) for path in resolved_images]
        if resolved_video:
            prompt.append(Video(resolved_video))
        prompt.append(text)
        ...
        return self.model.generate_content(prompt, generation_config=generation_config).strip()

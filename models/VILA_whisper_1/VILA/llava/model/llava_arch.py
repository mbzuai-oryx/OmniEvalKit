            ...
            if name == "image":
                if len(media["image"]) == 1 and self.config.image_aspect_ratio in ["dynamic", "dynamic_s2"]:
                    ...
                    if self.config.image_aspect_ratio == "dynamic":
                        images = process_image(media["image"][0], self.config, None, enable_dynamic_res=True).to(
                            dtype=vision_dtype
                        )
                    ...
                    images = process_images(media["image"], self.vision_tower.image_processor, self.config).to(
                        dtype=vision_dtype
                    )
                ...
                if self.config.image_aspect_ratio == "dynamic" and self.config.video_max_tiles > 1:
                    ...
                    media[name] = [
                        process_images(images, self.vision_tower.image_processor, self.config).to(dtype=vision_dtype)
                        for images in media[name]
                    ]
            else:
